...
- Base range expands from 10 to 50, fully ramped after 3 minutes of play (Sprint ends there; Endless then holds at full difficulty)
...
- [x] Randomness is sacred → **no shared / daily seed for single-player.** A fixed seed makes the next draw knowable (memorizable) and kills the "you don't know what's coming" tension. The luck of the draw is intentional; skill is board-agnostic (e.g. `×0` discards any starting board), so different-board scores already compare fairly — a shared board adds nothing. (This is unrelated to Battle, which deliberately shares a seed between the two players so the head-to-head prediction stream is fair — that's a fixed _pairing_, not a publicly-knowable daily draw.)
...
- [x] Open discovery space → an absolutely-safe, high-scoring mechanism may exist that even the author hasn't found, and that is acceptable by design. **Do not balance-patch to remove dominant strategies** — e.g. `×0`'s reset-and-prep-farm power is left as-is on purpose. This is distinct from fixing genuine bugs: a stale difficulty constant or a decimal mis-read as overflow are simply _wrong_ and do get fixed (they are not strategies)
